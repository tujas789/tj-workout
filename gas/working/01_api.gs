...
function progress_() {
  ...
  /* ความสม่ำเสมอ — นับเฉพาะเซสชันที่ทำจริง
     ★ คืน null (ไม่ใช่ 0) ให้แถวที่ไม่มี session_key เพราะ twoWindows_ นับ "จำนวนสมาชิก"
       ถ้าคืน 0 แถวขยะจะถูกนับเป็นเซสชันด้วย */
  const sess = twoWindows_(session, function (r) { return r.session_key ? 1 : null; });
